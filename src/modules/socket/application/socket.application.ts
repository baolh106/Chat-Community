...
import type { ISocketService } from "./socket.application.interface";

export class SocketApplication implements ISocketService {
  ...
  private async emitToUserRoleInRoomAsync(
    ...
    for (const remote of sockets) {
      const session = remote.data as SocketSessionData | undefined;
